...
import { CreateComponentDialog } from '@/components/admin/components/CreateComponentDialog';
...
export default function ComponentsPage() {
  ...
  async function handleSeedFlutterComponents() {
    setLoading(true);
    try {
      const response = await apiRequest<{ results: any[] }>('/admin/components/seed', {
        method: 'POST',
      });
      ...
      if (!response.success) {
        toast({
          title: 'Error',
          description: response.message || 'Failed to seed components',
          variant: 'destructive',
        });
        return;
      }
      ...
      toast({
        title: 'Success',
        description: `Successfully seeded ${response.data.results.length} Flutter components`,
      });
      ...
      toast({
        title: 'Error',
        description: 'Failed to seed components',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
      ...
      >
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={handleSeedFlutterComponents}
            disabled={loading}
          >
            {loading && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
            Seed Flutter Components
          </Button>
